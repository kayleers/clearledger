...
export default function TermsOfServiceContent() {
  const { sections, lastUpdated } = useTermsOfService();
    ...
    <div className="space-y-8 pb-6">
      ...
      {sections.slice(2).map((section, idx) => (
        <div key={idx} className="space-y-4">
          <h2 className="text-xl font-bold text-blue-700 flex items-center gap-3 mt-2">
            <span className="bg-blue-100 text-blue-800 w-9 h-9 rounded-full flex items-center justify-center text-base font-bold flex-shrink-0">
              {idx + 1}
            </span>
      ...
      <div className="pt-8 mt-8 border-t border-slate-200 text-center space-y-2">
        <p className="text-xs text-slate-600 font-medium">
          These terms are embedded within the ClearLedger app.
        </p>
