  ...
  try {
    ...
    const formatCurrency = (amount, currency = 'USD') => {
      ...
      return formatted
        ...
        .replace(/₹/g, 'INR ').replace(/₽/g, 'RUB ').replace(/₩/g, 'KRW ')
        .replace(/₪/g, 'ILS ').replace(/₱/g, 'PHP ').replace(/₺/g, 'TRY ')
        .replace(/฿/g, 'THB ').replace(/₫/g, 'VND ').replace(/zł/g, 'PLN ')
        .replace(/₴/g, 'UAH ').replace(/₡/g, 'CRC ').replace(/₦/g, 'NGN ')
        .replace(/₨/g, 'PKR ').replace(/₲/g, 'PYG ').replace(/₵/g, 'GHS ')
        .replace(/₸/g, 'KZT ').replace(/₼/g, 'AZN ').replace(/₮/g, 'MNT ')
        .replace(/元/g, 'CNY ').replace(/円/g, 'JPY ').replace(/원/g, 'KRW ');
    };
    ...
    if (cards.length > 0) {
      ...
      doc.text('Credit Cards', margin, yPos); yPos += 8;
      cards.forEach((card) => {
        checkNewPage(25);
    ...
    if (bankAccounts.length > 0) {
      checkNewPage(30);
      doc.setFontSize(16); doc.setTextColor(16, 185, 129);
      doc.text('Bank Accounts', margin, yPos); yPos += 8;
    ...
    if (loans.length > 0) {
      checkNewPage(30);
      doc.setFontSize(16); doc.setTextColor(16, 185, 129);
      doc.text('Loans & Mortgages', margin, yPos); yPos += 8;
      loans.forEach((loan) => {
        checkNewPage(25);
        doc.setFontSize(12); doc.setTextColor(0);
    ...
    if (bills.length > 0) {
      checkNewPage(30);
      doc.setFontSize(16); doc.setTextColor(16, 185, 129);
      doc.text('Recurring Bills', margin, yPos); yPos += 8;
      bills.forEach((bill) => {
        checkNewPage(20);
        doc.setFontSize(12); doc.setTextColor(0);
    ...
    if (deposits.length > 0) {
      checkNewPage(30);
      doc.setFontSize(16); doc.setTextColor(16, 185, 129);
      doc.text('Recurring Deposits', margin, yPos); yPos += 8;
    ...
    if (transfers.length > 0) {
      checkNewPage(30);
      doc.setFontSize(16); doc.setTextColor(16, 185, 129);
      doc.text('Bank Transfers', margin, yPos); yPos += 8;
    ...
    if (conversions.length > 0) {
      checkNewPage(30);
      doc.setFontSize(16); doc.setTextColor(16, 185, 129);
      doc.text('Currency FX', margin, yPos); yPos += 8;
      conversions.forEach((conversion) => {
        checkNewPage(20);
        doc.setFontSize(12); doc.setTextColor(0);
        doc.text(conversion.name, margin + 5, yPos);
        doc.setFontSize(10); doc.setTextColor(100);
        doc.text(`${conversion.from_currency} to ${conversion.to_currency}`, margin + 10, yPos + 5);
    ...
    checkNewPage(30);
    doc.setFontSize(16); doc.setTextColor(16, 185, 129);
    doc.text(`Payment Schedule - ${monthNames[now.getMonth()]} ${now.getFullYear()}`, margin, yPos);
    yPos += 8;
    ...
    const payments = [];
    cards.forEach(card => {
      if (card.due_date) payments.push({ date: card.due_date, name: card.name, type: 'Credit Card', amount: card.projected_monthly_payment || card.min_payment || 0, currency: card.currency, account: card.bank_account_id ? getAccountName(card.bank_account_id) : '', isOutflow: true });
    });
    bills.forEach(bill => {
      if (bill.frequency === 'monthly' && bill.due_date) payments.push({ date: bill.due_date, name: bill.name, type: 'Bill', amount: bill.amount, currency: bill.currency, account: bill.bank_account_id ? getAccountName(bill.bank_account_id) : '', isOutflow: true });
    });
    loans.forEach(loan => {
      if (loan.payment_due_date) payments.push({ date: loan.payment_due_date, name: loan.name, type: 'Loan', amount: loan.projected_monthly_payment || loan.monthly_payment || 0, currency: loan.currency, account: loan.bank_account_id ? getAccountName(loan.bank_account_id) : '', isOutflow: true });
    });
    payments.sort((a, b) => a.date - b.date);

    const paymentsByDate = {};
    payments.forEach(p => { if (!paymentsByDate[p.date]) paymentsByDate[p.date] = []; paymentsByDate[p.date].push(p); });

    Object.entries(paymentsByDate).forEach(([date, items]) => {
      checkNewPage(15 + items.length * 8);
      doc.setFontSize(11); doc.setTextColor(60, 60, 60);
      doc.text(`Day ${date}`, margin + 5, yPos); yPos += 6;
      items.forEach(item => {
        ...
        doc.text(formatCurrency(item.amount, item.currency), 190, yPos, { align: 'right' });
        doc.setTextColor(40, 40, 40);
        yPos += 5;
        if (item.account) {
          doc.setFontSize(7); doc.setTextColor(100, 100, 100);
          doc.text(`    From: ${item.account}`, margin + 8, yPos);
          yPos += 5;
        }
      });
      yPos += 3;
    ...
    doc.addPage(); yPos = 20;
    doc.setFontSize(16); doc.setTextColor(16, 185, 129);
    doc.text('Summary', margin, yPos); yPos += 10;
    doc.setFontSize(12); doc.setTextColor(0);
    doc.text(`Total Credit Cards: ${cards.length}`, margin + 5, yPos); yPos += 7;
    doc.text(`Total Bank Accounts: ${bankAccounts.length}`, margin + 5, yPos); yPos += 7;
    doc.text(`Total Loans: ${loans.length}`, margin + 5, yPos); yPos += 7;
    doc.text(`Total Recurring Bills: ${bills.length}`, margin + 5, yPos); yPos += 7;
    doc.text(`Total Recurring Deposits: ${deposits.length}`, margin + 5, yPos);

    yPos += 15;
    doc.setFontSize(14); doc.setTextColor(16, 185, 129);
    doc.text('Balances by Currency', margin, yPos); yPos += 8;
    const balances = {};
    cards.forEach(card => { const curr = card.currency || 'USD'; balances[curr] = (balances[curr] || 0) + card.balance; });
    const assets = {};
    bankAccounts.forEach(acc => { const curr = acc.currency || 'USD'; assets[curr] = (assets[curr] || 0) + (acc.balance || 0) + (acc.stocks_investments || 0); });
    doc.setFontSize(11); doc.setTextColor(0);
    doc.text('Credit Card Debt:', margin + 5, yPos); yPos += 6;
    Object.entries(balances).forEach(([curr, total]) => { doc.setFontSize(10); doc.setTextColor(100); doc.text(`${curr}: ${formatCurrency(total, curr)}`, margin + 10, yPos); yPos += 5; });
    yPos += 5;
    doc.setFontSize(11); doc.setTextColor(0);
    doc.text('Bank Account Assets:', margin + 5, yPos); yPos += 6;
    Object.entries(assets).forEach(([curr, total]) => { doc.setFontSize(10); doc.setTextColor(100); doc.text(`${curr}: ${formatCurrency(total, curr)}`, margin + 10, yPos); yPos += 5; });
